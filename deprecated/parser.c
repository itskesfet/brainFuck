...
*
*/
#define TOKEN_FILE "parsed_token_file.txt"
 ...
 *
 */
Token  *parser(const char *buffer, size_t len , size_t *res_count){
	assert(len > 0);
	int main_counter = 0;
	Token	*tokens = lexer(buffer, len, &main_counter);
	for (int i = 0; i < main_counter; i++) {
	    printf("Index: %d | Type: %d | Value: %d | Jump: %d\n",
	           i,
	           tokens[i].type,
	           tokens[i].value,
	           tokens[i].jump);
	...
	//LOOP
		for(int i = 0; i < main_counter; i++){
			...
			if(T == INC_MEM_PTR || T == DEC_MEM_PTR || T == INC_BLK_VAL || T == DEC_BLK_VAL ){	
				tok[tok_size].type 	= T; 
	...
	//---JUMP---	
			//tokens[i].jump = -1;
			if(T == CUR_BLK_OPEN){
				cur_blk_loc[++blk_loc_pos] = shorted_tokens_ptr;
				shorted_tokens[shorted_tokens_ptr].type 	= tokens[i].type;
				shorted_tokens[shorted_tokens_ptr].value 	=  1;
				shorted_tokens[shorted_tokens_ptr].jump		= -2; //[	
				shorted_tokens_ptr++;
				has_token++;
				continue;
			}
			if(T == CUR_BLK_CLOSE){
				if(blk_loc_pos < 0){
    				...
    				int open_idx = cur_blk_loc[blk_loc_pos--];
    				int close_idx = shorted_tokens_ptr;

    				shorted_tokens[shorted_tokens_ptr].type  = T;
    				shorted_tokens[shorted_tokens_ptr].value = 1;
    				shorted_tokens[shorted_tokens_ptr].jump  = open_idx;
    				shorted_tokens[open_idx].jump 		 = close_idx;
    				shorted_tokens_ptr++;
				continue;
			}
	//---
	...
	//---COMPRESSION---
			...
			if(has_token == 0){
				//tok[0].value++;
				shorted_tokens[shorted_tokens_ptr].type 	= tok[0].type;
				shorted_tokens[shorted_tokens_ptr].value 	= tok[0].value;
				shorted_tokens[shorted_tokens_ptr].jump		= tok[0].jump;	
				shorted_tokens_ptr++;
				has_token++;
			}
			...
			if( k > 0 && tok_size > 1 
				 ...
				 *
				 */
				if( tok_size > 1 && tok[tok_size - 2].type != tok[tok_size - 1].type ){
	...
	(*res_count)	=(size_t)shorted_tokens_ptr;
	//LOOP END
